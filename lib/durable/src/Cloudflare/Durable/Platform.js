...
/**
 * @typedef {{ sql: string, bindings: unknown[] }} SqlCommand
 */

/**
 * Run a group of SQL writes in one synchronous storage transaction.
 *
 * @param {DurableObjectState} ctx
 * @returns {(commands: SqlCommand[]) => Effect<Record<string, SqlStorageValue>[][]>}
 */
export const sqlBatch = (ctx) => (commands) => () =>
  ctx.storage.transactionSync(() =>
    commands.map(({ sql, bindings }) =>
      ctx.storage.sql.exec(sql, ...bindings.map(bindable)).toArray()
    )
  );
